...
#and https://github.com/DebasishDhal/Thesis_Repository/blob/main/results/cloudsat/cloudpresence/year2013%25clearisonland.png, taking full year of 2013 into consideration at once.
...
#Plotting the cloud presence map for the whole year

dfwholeyear = pd.concat(groupedmonthlycollection.values(), ignore_index=True)
print(len(dfwholeyear))
...
clear_land_pixels = len(result[(result['landseaflagchar']=='Land') & (result['cloudyornotchar']=='Clear')])
cloudy_land_pixels = len(result[(result['landseaflagchar']=='Land') & (result['cloudyornotchar']=='Cloudy')])
clear_ocean_pixels = len(result[(result['landseaflagchar']=='Ocean') & (result['cloudyornotchar']=='Clear')])
cloudy_ocean_pixels = len(result[(result['landseaflagchar']=='Ocean') & (result['cloudyornotchar']=='Cloudy')])
...
#Calculate the percentage of clear pixels over land and ocean, and cloudy pixels over land and ocean
...
ax.set_title('Percentage of Clear & Cloudy Pixels - Land vs Ocean \nFull Year 2013, #Pixels: {}, #Orbits: {}'.format(len(result),len(result['orbit'].unique())), fontsize=15)
ax.legend()
