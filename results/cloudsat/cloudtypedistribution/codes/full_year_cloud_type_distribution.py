...
#A function that takes a single file and returns stats of the cloud layers
...
#This function is applied on all the files for the year 2013. year - month - day_number - 14_to_16_files_per_day
import re
...
cloudtype_dict={1:'Cirrus',2:'Altostratus',3:'Altocumulus',4:'Stratus',5:'Stratocumulus',6:'Cumulus',7:'Nimbostratus'
                ,8:'Deep convective',9:'Unknown'} #Code used by CloudSat for different cloud layers
...
#Figure with 4 subplots 2x2. Plot the global fractional abundance in first two, and the fractional abundance in the area of interest in the last two
fig,ax=plt.subplots(2,2,figsize=(10,10))
plt.figtext(0.5,1.0050,'Global data, datapoints = {}'.format(len(dfcopy)),ha='center', va='center',fontsize=16)
#Plot the fractional abundance of each cloud type in the dataset
dfcopy = df.copy()
dfcopy['latbin']=np.floor(dfcopy['lat'])
...
for i, c in enumerate(colors_subplot_1):
    ...
    ax[1,0].legend()
plt.figtext(0.5,0.51,'Over Indian subcontinent region, datapoints = {}'.format(len(dfcopy_aoi)),ha='center', va='center',fontsize=16)
for i, c in enumerate(colors_subplot_2):
...
axproj = fig.add_axes([0.565, 0.380, 0.1, 0.1], projection=projection)
axproj.set_extent([lon_left, lon_right, lat_bot, lat_top], crs=projection)
axproj.stock_img()
axproj.coastlines()
axproj.add_feature(cfeature.BORDERS)

#Make their scales the same, so that the fractional abundance of each cloud type can be compared

y_lim=[0,0]
y_lim[0] = min(ax[0,0].get_ylim()[0],ax[0,1].get_ylim()[0],ax[1,0].get_ylim()[0],ax[1,1].get_ylim()[0])
y_lim[1] = max(ax[0,0].get_ylim()[1],ax[0,1].get_ylim()[1],ax[1,0].get_ylim()[1],ax[1,1].get_ylim()[1])
...
plt.figtext(0.5,1.051,'Cloud Type distribution as a Function of Latitude \n (Full Year, 2013)',ha='center', va='center',fontsize=16,color='red')
plt.tight_layout()
